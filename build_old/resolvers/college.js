...
const Mutation = {
    ...
    createCollege: async (root, { input: { name, fullName, createdBy, description, image } }, { College, authUser }) => {
        ...
        if (image) {
            ...
            let uploadImage;
            try {
                uploadImage = await (0, s3_bucket_1.uploadToS3Bucket)(stream, 'college', (0, image_transform_1.transformer)({ width: 420 }));
            }
            catch (err) {
     ...
     * @param {string} description
     */
    updateCollege: async (root, { input: { id, name, fullName, updatedBy, description, image } }, { College, authUser }) => {
        if (!id || !updatedBy) {
     ...
     */
    deleteCollege: async (root, { input: { id, imagePublicId } }, { College, Programme, Course, User, Post, authUser }) => {
        if (!id) {
            throw new Error('id param is required.');
        }
        (0, user_1.assertAdmin)(authUser);
        // only allow deletion if there are no collection refrencing this college
        const cannotDelete = (await Programme.findOne({ college: id })) ||
