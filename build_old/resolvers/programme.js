...
const Mutation = {
     ...
     * @param {string} collegeId
     */
    createProgramme: async (root, { input: { name, fullName, createdBy, description, image, collegeId, degree, termType, termsCount } }, { Programme, College, authUser }) => {
        if (!name || !fullName || !createdBy || !collegeId) {
            throw new Error('name, fullName, createdBy, collegeId params are required.');
        }
        (0, user_1.assertAdmin)(authUser);
        ...
        if (image) {
            ...
            let uploadImage;
            try {
                uploadImage = await (0, s3_bucket_1.uploadToS3Bucket)(stream, 'programme', (0, image_transform_1.transformer)({ width: 420 }));
            }
            catch (err) {
        ...
        await College.findOneAndUpdate({ _id: collegeId }, { $push: { programmes: newProgramme.id }, $inc: { programmesCount: 1 } });
        return newProgramme;
    },
    /**
     ...
     * @param {string} description
     */
    updateProgramme: async (root, { input: { id, name, fullName, updatedBy, description, image, degree, termType, termsCount } }, { Programme, authUser }) => {
        if (!id || !updatedBy) {
        ...
        const programme = await Programme.findOneAndUpdate({ _id: id }, {
            name,
            fullName,
            updatedBy,
            description,
     ...
     */
    deleteProgramme: async (root, { input: { id, imagePublicId } }, { Programme, College, Course, User, Post, authUser }) => {
        if (!id) {
            throw new Error('id param is required.');
        }
        (0, user_1.assertAdmin)(authUser);
        // only allow deletion if there are no collection refrencing this programme
        const cannotDelete = (await Course.findOne({ programme: id })) ||
        ...
        if (imagePublicId) {
            try {
                await (0, s3_bucket_1.deleteFromS3Bucket)(imagePublicId);
            }
            catch (err) {
        ...
        await College.findOneAndUpdate({ _id: programme.college.id }, { $pull: { programmes: programme.id }, $inc: { programmesCount: -1 } });
        return programme;
    },
    /**
