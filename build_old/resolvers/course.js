...
const Mutation = {
     ...
     * @param {string} programmeId
     */
    createCourse: async (root, { input: { name, fullName, term, createdBy, description, image, collegeId, programmeId } }, { Course, Programme, authUser }) => {
        ...
        if (image) {
            ...
            let uploadImage;
            try {
                uploadImage = await (0, s3_bucket_1.uploadToS3Bucket)(stream, 'course', (0, image_transform_1.transformer)({ width: 420 }));
            }
            catch (err) {
        ...
        await Programme.findOneAndUpdate({ _id: programmeId }, { $push: { courses: newCourse.id }, $inc: { coursesCount: 1 } });
        return newCourse;
    },
    /**
     * Deletes a course
     ...
     * @param {string} description
     */
    updateCourse: async (root, { input: { id, name, fullName, term, updatedBy, description, image } }, { Course, authUser }) => {
        if (!id || !updatedBy) {
    ...
    deleteCourse: async (root, { input: { id, imagePublicId } }, { Course, Programme, User, Post, authUser }) => {
        ...
        await Programme.findOneAndUpdate({ _id: course.programme.id }, { $pull: { courses: course.id }, $inc: { coursesCount: -1 } });
        return course;
    },
    /**
