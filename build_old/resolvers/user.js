...
exports.assertAdmin = exports.assertAuthenticated = void 0;
const mongoose_1 = __importDefault(require("mongoose"));
const apollo_server_1 = require("apollo-server");
const s3_bucket_1 = require("../utils/s3-bucket");
const apollo_server_2 = require("../utils/apollo-server");
const image_transform_1 = require("../utils/image-transform");
const Subscriptions_1 = require("../constants/Subscriptions");
const types_1 = require("../constants/types");
const assertAuthenticated = (authUser) => {
    if (!authUser) {
        throw new apollo_server_1.AuthenticationError('You need to be logged in');
    }
};
exports.assertAuthenticated = assertAuthenticated;
const assertAdmin = (authUser) => {
    (0, exports.assertAuthenticated)(authUser);
    if (types_1.UserRole.Admin > authUser.role) {
        throw new apollo_server_1.AuthenticationError('You need to be admin');
    }
};
...
const Query = {
    ...
    getAuthUser: async (root, args, { authUser, Message, User }) => {
        ...
        const lastUnseenMessages = await Message.aggregate([
            {
                $match: {
                    receiver: mongoose_1.default.Types.ObjectId(authUser.id),
                    seen: false,
                },
            },
            {
        ...
        const sortedConversations = newConversations.sort((a, b) => b.lastMessageCreatedAt.toString().localeCompare(a.lastMessageCreatedAt));
...
const Mutation = {
    ...
    uploadUserPhoto: async (root, { input: { id, image, imagePublicId, isCover } }, { User }) => {
        ...
        let uploadImage;
        try {
            uploadImage = await (0, s3_bucket_1.uploadToS3Bucket)(stream, 'user', (0, image_transform_1.transformer)({ width: 176 }), imagePublicId);
        }
        catch (err) {
    ...
    updateUserCollege: async (root, { input: { id, collegeId } }, { User, College }) => {
        ...
        if (user.college) {
            ...
            await College.findOneAndUpdate({ _id: user.college.id }, { $pull: { students: user.id }, $inc: { studentsCount: -1 } });
        }
    ...
    updateUserProgramme: async (root, { input: { id, programmeId } }, { User, Programme }) => {
        ...
        if (user.programme) {
            ...
            await Programme.findOneAndUpdate({ _id: user.programme.id }, { $pull: { students: user.id }, $inc: { studentsCount: -1 } });
        }
...
const Subscription = {
     ...
     */
    isUserOnline: {
        subscribe: (0, apollo_server_1.withFilter)(() => apollo_server_2.pubSub.asyncIterator(Subscriptions_1.Subscriptions.Is_User_Online), (payload, variables, { authUser }) => variables.authUserId === authUserId),
    },
};
