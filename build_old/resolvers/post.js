...
const Mutation = {
     ...
     */
    createPost: async (root, { input: { title, pdf, images, authorId, collegeId, programmeId, courseId, fileName } }, { Post, User, Course }) => {
        if (!authorId) {
        ...
        if (pdf) {
            ...
            try {
                uploadPDF = await (0, s3_bucket_1.uploadPDFToS3Bucket)(stream, `post/${fileExt}`, fileName);
            }
            catch (err) {
            ...
            await Promise.all(images)
                ...
                .then((streams) => Promise.all(streams.map((stream) => (0, s3_bucket_1.uploadToS3Bucket)(stream, 'post/images', (0, image_transform_1.transformer)({ width: 600 })))))
                .then((responses) => {
     ...
     */
    deletePost: async (root, { input: { id, pdfPublicId, imagePublicIds } }, { Post, Course, Like, User, Comment, Notification }) => {
        // Remove post pdf from s3 bucket, if pdfPublicId is present
        if (pdfPublicId) {
            (0, s3_bucket_1.deletePDFFromS3Bucket)(pdfPublicId).catch((err) => {
                throw err;
            });
        }
        // Remove post image from s3 bucket, if imagePublicIds is present
        if (imagePublicIds) {
            await Promise.all(imagePublicIds.map((pid) => (0, s3_bucket_1.deleteFromS3Bucket)(pid))).catch((err) => {
                throw err;
            });
        }
